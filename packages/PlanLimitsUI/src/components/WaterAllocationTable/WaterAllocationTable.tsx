...
export const WaterAllocationTable = ({ council }: { council: Council }) => {
  ...
  const tableData = (useWaterAllocationQuery(council.id, waterType, monthStart)
    .data || []) as GroundwaterAllocation[] | SurfaceWaterAllocation[];
  ...
  const columns: ColumnDescriptor[] = [
    { name: 'plan_region_id', visible: false },
    {
      ...
      type: 'percent',
      formula: 'percent(total_allocation, allocation_limit)',
      valueOk: [(value: DataValueType) => numValue(value) <= 100],
      heading: 'NRP Allocation %',
    },
    ...
    { name: 'allocation_limit', scale: 3, scaleSymbol: 'k' },
    {
      ...
      highlight: (c: string) => `border-r-2 border-l-2 border-${c}`,
    },
  ...
  const outerFilters: FilterDescriptor[] = useMemo(() => {
    const filters = [
      {
        ...
        options: [
          { label: 'Groundwater', value: 'ground' },
          { label: 'Surface water', value: 'surface' },
        ],
        onChange: (waterType: WaterType) =>
          setWaterType(waterType as WaterType),
    ...
    if (compareColumn && tableData && monthStart.length > 1)
      filters.push({
        ...
        options: getOrder(waterType)
          .filter((c) => !['name'].includes(c))
          .map((c) => ({
            label: columns.find((cc) => cc.name == c)?.heading || capitalise(c),
            value: c,
          })),
        onChange: (compareCol: unknown) =>
          setCompareColumn(compareCol as string),
        suppressSelectAll: true,
        className: 'pl-0 pr-0',
        label: 'comparing',
      });
