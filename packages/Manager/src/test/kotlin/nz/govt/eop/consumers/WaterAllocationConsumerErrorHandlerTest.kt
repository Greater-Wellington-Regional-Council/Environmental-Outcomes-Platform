...
class WaterAllocationConsumerErrorHandlerTest(
  ...
  fun `Should send message to DLT when processing fails multiple times`() {
    ...
    val firstMessage = WaterAllocationMessage("poison", BigDecimal("100.11"), "ingest-id", Instant.now())
    val secondMessage = WaterAllocationMessage("areaid", BigDecimal("100.11"), "ingest-id", Instant.now())
