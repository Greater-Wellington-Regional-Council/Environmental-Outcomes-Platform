...
class FreshwaterManagementUnitsControllerTest {
    ...
    fun `Get freshwater-management-units-lng-lat`() {
        ...
        val fmu = FreshwaterManagementUnit(id = 1, fmuGroup = "Western hill rivers", tangataWhenuaSites = listOf(
            TangataWhenuaSite(
                id = 1,
        ...
        mvc.perform(
            MockMvcRequestBuilders.get("/freshwater-management-units/by-lng-and-lat?lng=175.34&lat=-41")
    ...
    fun `Search freshwater-management-units intersecting a shape`() {
        ...
        Mockito.`when`(fmuService.findFreshwaterManagementUnitsByShape(ArgumentMatchers.anyString(), ArgumentMatchers.anyBoolean()))
            .thenReturn(listOf(fmu1, fmu2))

        mvc.perform(
            MockMvcRequestBuilders.post("/freshwater-management-units/search-for-freshwater-managements-intersecting")
                .contentType(MediaType.APPLICATION_JSON)
                .content(geoJson),
        )
            .andExpect(MockMvcResultMatchers.status().isOk)
