...
fun createUnsavedFMU(): FreshwaterManagementUnit {
    ...
    val geometryJson: String =
        objectMapper.writeValueAsString(mockFeatureResponse.features.find { feature -> feature.properties["FID"] == 20 }!!.geometry)

    val geometry = GeoJsonReader().read(geometryJson).also { g: Geometry ->
    ...
    return FreshwaterManagementUnit(
        ...
        farmPlanInfo = FarmPlanInfo().apply {
...
class FreshwaterManagementUnitTest(
    ...
    test("build a freshwater management unit") {
        ...
        savedFmu.farmPlanInfo.implementationIdeas shouldBe unsavedFMU.farmPlanInfo.implementationIdeas

        savedFmu.geom shouldBe unsavedFMU.geom
        savedFmu.boundary shouldNotBe null
    }

    test("toFeature() should correctly convert FreshwaterManagementUnit to FreshwaterManagementUnitFeature") {
