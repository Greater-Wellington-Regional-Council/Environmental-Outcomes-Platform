...
@SpringBootTest
@Transactional
class WaterAllocationAndUsageViewsTest(
    ...
    fun `should be empty with no allocations`() {
        ...
        val result =
            jdbcTemplate.queryForObject(
                "select count(*) from water_allocation_and_usage_by_area",
                Int::class.java,
            )
    ...
    private fun createWaterReadings(
        ...
        values.forEach {
            createWaterMeterReading(
                testSiteId,
                it,
                observationDate.plusHours(++h).toInstant(ZoneOffset.UTC),
            )
        }
    ...
    fun `should aggregate allocation data for different areas separately`() {
        ...
        createTestObservation(
            secondAllocationInSameArea.meters[0].toInt(),
            5,
            observationDate.toInstant(ZoneOffset.UTC),
        )
        createTestObservation(
            secondAllocationInSameArea.meters[1].toInt(),
            5,
            observationDate.toInstant(ZoneOffset.UTC),
        )
        createTestObservation(
