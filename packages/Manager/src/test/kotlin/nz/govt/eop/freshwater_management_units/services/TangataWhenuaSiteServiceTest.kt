...
class TangataWhenuaSiteServiceTest : BehaviorSpec({
    given("TangataWhenuaSiteServicesTest") {
        ...
        val service = TangataWhenuaSiteService(restTemplateMock, repositoryMock).apply {
        ...
        `when`("loadFromArcGIS is called") {
            ...
            then("it should delete all existing sites") {
                verify(repositoryMock).deleteAll()
            }

            then("it should fetch feature collection from the configured URL") {
                verify(restTemplateMock).getForEntity(URI.create("http://test.url1"), FeatureCollection::class.java)
            }
            ...
            then("it should save each fetched feature as a Tangata Whenua site") {
                ...
                verify(repositoryMock).saveWithGeom(anyString(), anyList(), anyString(), anyString(), anyString())
            }
        }
    }
})