...
CREATE TABLE whaitua_boundaries
(
...
CREATE INDEX ON rivers (created_at DESC);
...
CREATE INDEX ON watersheds USING gist (geom);

CREATE INDEX ON watersheds (created_at DESC);
...
CREATE INDEX ON minimum_flow_limits (created_at DESC);


CREATE TABLE minimum_flow_limit_boundaries
(
  id                 INTEGER   NOT NULL,
  hydro_id           INTEGER   NOT NULL,
...
CREATE INDEX ON minimum_flow_limit_boundaries (created_at DESC);
...
CREATE TABLE allocation_amounts
(
  id                                                  INTEGER UNIQUE,
...
CREATE INDEX ON allocation_amounts (created_at DESC);


CREATE TABLE surface_water_management_boundaries
(
  id                 INTEGER   NOT NULL,
  hydro_id           INTEGER   NOT NULL,
  excluded_hydro_ids INTEGER[] NOT NULL,
...
CREATE INDEX ON surface_water_management_boundaries (created_at DESC);
...
CREATE INDEX ON raw_rec_features_rivers (ingested_at DESC);
...
CREATE INDEX ON raw_rec_features_watersheds (ingested_at DESC);
...
CREATE INDEX ON raw_rec_rivers_names (ingested_at DESC);
