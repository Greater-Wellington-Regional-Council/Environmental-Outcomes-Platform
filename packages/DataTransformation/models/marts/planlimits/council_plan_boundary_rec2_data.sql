...
all_hydro_ids AS (
    ...
    INNER JOIN expanded_input AS a
    ...
    SELECT
        ...
        rv.hydro_id
    FROM {{ ref('rivers') }} AS rv
    INNER JOIN all_hydro_ids AS a
        ON a.hydro_id = rv.next_hydro_id
    WHERE 
        NOT EXISTS (
            SELECT 1 FROM excluded_hydro
        ) 
        OR rv.hydro_id NOT IN (
            SELECT excluded_hydro_id FROM excluded_hydro
            WHERE excluded_hydro.source_id = a.source_id
        )
),
...
as_grouped_boundaries AS (
    ...
    FROM with_watershed_geom
    GROUP BY council_id, source_id
)

SELECT *
FROM as_grouped_boundaries;
